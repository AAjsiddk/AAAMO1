...
export default function SettingsPage() {
  ...
  const handleColorChange = useCallback((colorSetter: React.Dispatch<React.SetStateAction<string>>, cssVar: string, newValue: string) => {
  ...
  const handleAccountUpdate = async (e: React.FormEvent) => {
      ...
      try {
          ...
          if (error.code === 'auth/wrong-password') desc = "كلمة المرور الحالية غير صحيحة.";
  ...
  if (!isMounted) {
      return (<div className="flex-1 space-y-4 p-4 md:p-8 pt-6"><div className="flex items-center justify-center py-16"><Loader2 className="h-8 w-8 animate-spin text-muted-foreground" /></div></div>);
  }
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-end">
                ...
                <div className="space-y-2"><Label htmlFor="backgroundColor">لون الخلفية (Background)</Label><ColorPicker value={backgroundColor} onChange={(color) => handleColorChange(setBackgroundColor, '--background', color)} /></div>
                <div className="space-y-2"><Label htmlFor="accentColor">اللون الثانوي (Accent)</Label><ColorPicker value={accentColor} onChange={(color) => handleColorChange(setAccentColor, '--accent', color)} /></div>
            </div>
            <Button onClick={saveTheme} disabled={isSaving}>{isSaving && <Loader2 className="mr-2 h-4 w-4 animate-spin" />} حفظ تغييرات المظهر</Button>
        </CardContent>
      </Card>
      <Card>
        <CardHeader><CardTitle className="flex items-center gap-2"><Lock /> إعدادات الأمان</CardTitle><CardDescription>قم بتحديث كلمة المرور الخاصة بحسابك.</CardDescription></CardHeader>
        <CardContent>
            <form onSubmit={handleAccountUpdate} className="space-y-4 max-w-sm">
                <div className="space-y-2"><Label htmlFor="currentPassword">كلمة المرور الحالية</Label><Input id="currentPassword" type="password" value={currentPassword} onChange={(e) => setCurrentPassword(e.target.value)} placeholder="أدخل كلمة المرور الحالية" /></div>
                <div className="space-y-2"><Label htmlFor="newPassword">كلمة المرور الجديدة</Label><Input id="newPassword" type="password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} placeholder="أدخل كلمة المرور الجديدة" /></div>
                <Button type="submit" disabled={isSaving || !newPassword}>{isSaving && <Loader2 className="mr-2 h-4 w-4 animate-spin" />} تحديث كلمة المرور</Button>
            </form>
        </CardContent>
      </Card>
    </div>
