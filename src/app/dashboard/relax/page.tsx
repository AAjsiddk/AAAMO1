...
import { PlusCircle, Trash2, Loader2, Inbox, GripVertical, Plus, Edit, Check, X, Minus, Pin, PinOff, Wind } from 'lucide-react';
...
export default function RelaxPage() {
  ...
  const [editingPlan, setEditingPlan] = useState<RelaxationActivity | null>(null);
  ...
  const plansQuery = useMemoFirebase(() => plansCollectionRef ? query(plansCollectionRef, orderBy('order', 'asc')) : null, [plansCollectionRef]);
  ...
  const handleOpenDialog = (plan: RelaxationActivity | null) => {
    setEditingPlan(plan);
  ...
  const handleStatusChange = async (
    ...
    const planRef = doc(firestore, `users/${user.uid}/relaxationActivities`, ids.planId);
  ...
  const handleSubmit = async (values: { title: string }) => {
    ...
    try {
      if (editingPlan) {
        const planRef = doc(firestore, `users/${user.uid}/relaxationActivities`, editingPlan.id);
  ...
  const handleSubtaskSubmit = async (values: { title: string }) => {
    ...
    setIsSubmitting(true);
    try {
        const newSubtask: RelaxationSubtask = {
        ...
        const planRef = doc(firestore, `users/${user.uid}/relaxationActivities`, parentPlanId);
        await updateDoc(planRef, { subtasks: newSubtasks });
  ...
  const StatusButtons = ({ type, ids, currentStatus }: { type: 'plan' | 'subtask', ids: {planId: string, subtaskId?: string}, currentStatus: RelaxationActivity['status']}) => (
        ...
        <DialogContent>
          ...
          <Form {...form}><form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4">
            <FormField control={form.control} name="title" render={({ field }) => (
              <FormItem><FormLabel>عنوان الخطة</FormLabel><FormControl><Input placeholder="مثال: رحلة نهاية الأسبوع" {...field} /></FormControl><FormMessage /></FormItem>
            )} />
            <DialogFooter>
      ...
      <Dialog open={isSubtaskDialogOpen} onOpenChange={setIsSubtaskDialogOpen}>
        <DialogContent>
          ...
          <Form {...form}><form onSubmit={form.handleSubmit(handleSubtaskSubmit)} className="space-y-4">
            <FormField control={form.control} name="title" render={({ field }) => (
              <FormItem><FormLabel>عنوان المهمة</FormLabel><FormControl><Input placeholder="مثال: حجز الفندق" {...field} /></FormControl><FormMessage /></FormItem>
            )} />
            <DialogFooter>
              <DialogClose asChild><Button type="button" variant="secondary">إلغاء</Button></DialogClose>
